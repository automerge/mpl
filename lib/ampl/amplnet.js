    ...
    value: function connect(config) {
      ...
      this.peergroup.on('peer', function (peer) {
        ...
        // END PEER STATS
          ...
          if (peer.self == false) {
            peer.send({ docId: _this2.store.getState().docId, vectorClock: _tesseract2.default.getVClock(_this2.store.getState()), seq: 0 });
          }
        });
        ...
        peer.on('message', function (m) {
          ...
          if (m.docId != _this2.store.getState().docId) {
            return;
          }
