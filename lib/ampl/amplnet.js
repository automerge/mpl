  ...
  _createClass(aMPLNet, [{
    ...
    value: function locatePeersThroughFriends(peer, knownPeers) {
      ...
      var _loop = function _loop(i) {
        ...
        if (!(remotePeerId in _this4.peerStats) && knownPeers[remotePeerId].connected && remotePeerId < _this4.SELF.id) {
    ...
    value: function broadcastKnownPeers() {
      ...
      this.peergroup.peers().forEach(function (peer) {
        console.log("Broadcasting known peers to " + peer.id, Object.keys(_this5.peerStats));
        peer.send({ knownPeers: _this5.peerStats });
      });
    }
  }, {
    key: 'broadcastState',
      ...
      if (action == "APPLY_DELTAS") {
        this.peergroup.peers().forEach(function (peer) {
          try {
            ...
            _this6.peerStats[peer.id].messagesSent += 1;
          } catch (e) {
