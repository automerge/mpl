    ...
    value: function connect(config) {
      ...
      if (this.doc_id) {
        ...
        if (process.env.SLACK_BOT_TOKEN) {
          ...
          bot = new _bonjourSignaler2.default({ doc_id: this.doc_id, name: this.name, session: this.peer_id });
        }
      ...
      var _loop = function _loop(i) {
        ...
        if (!(remotePeerId in _this4.peers) && knownPeers[remotePeerId].connected && remotePeerId < _this4.SELF.id) {
          ...
          var msg = { action: "hello", session: ids[i], name: knownPeers[remotePeerId].name
            ...
            var offerMsg = { action: "offer", name: _this4.SELF.name, session: _this4.SELF.id, doc_id: _this4.doc_id, to: remotePeerId, body: offer };
            peer.send(offerMsg);
          });
        }
      };
