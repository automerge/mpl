...
var BonjourSignaller = function (_EventEmitter) {
  ...
  function BonjourSignaller(peergroup, name, session) {
    ...
    if (!session) {
      console.log("NO SESSION PASSED TO BONJOUR SIGNALLER, MAKING ONE UP");
    }
    _this.SESSION = session || uuidv4();

    if (!name) {
      console.log("NO NAME PASSED TO BONJOUR SIGNALLER, MAKING ONE UP");
    }
  ...
  _createClass(BonjourSignaller, [{
    ...
    value: function publishBonjour() {
      ...
      var text = { session: this.SESSION, name: this.NAME };
    ...
    key: 'manualHello',
    value: function manualHello(host, port) {
      var _this4 = this;

      console.log("sendOffer():", host + ":" + port);
      var msg = { name: this.NAME, session: this.SESSION, action: 'hello'
      ...
      ws.on('message', function (data) {
        ...
        _this4.hearHello(greeting.name, greeting.session, host, port);
      });
    }
    ...
    value: function hearOffer(ws, signal) {
      ...
      this.peergroup.processSignal(meta, signal.body, function (reply) {
        var msg = { name: _this8.NAME, session: _this8.SESSION, body: reply, action: 'reply' };
