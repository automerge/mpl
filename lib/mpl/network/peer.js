...
var Peer = function (_EventEmitter) {
  ...
  _createClass(Peer, [{
    ...
    value: function processMessage(msg) {
      if (!_config2.default.encryptionKey) throw 'Please configure an encryption key';
      var encrypted = Buffer.from(msg.data, 'base64');
      var decrypted = Buffer.from(_libsodiumWrappers2.default.crypto_secretbox_open_easy(encrypted.slice(_libsodiumWrappers2.default.crypto_box_NONCEBYTES), encrypted.slice(0, _libsodiumWrappers2.default.crypto_box_NONCEBYTES), Buffer.from(_config2.default.encryptionKey, 'hex'), 'uint8array'));
      var data = _lz2.default.decode(decrypted).toString('utf8');
    ...
    value: function send(message) {
      ...
      if (!_config2.default.encryptionKey) throw 'Please configure an encryption key';
      var secret = Buffer.from(_config2.default.encryptionKey, 'hex');
      var nonce = Buffer.from(_libsodiumWrappers2.default.randombytes_buf(_libsodiumWrappers2.default.crypto_box_NONCEBYTES));
      var encrypted = Buffer.concat([nonce, Buffer.from(_libsodiumWrappers2.default.crypto_secretbox_easy(compressed, nonce, secret))]);
